...
def save_high_score(score):
    try:
        ...
        print(
            f"Warning: Could not save high score to {HIGH_SCORE_FILE}")
...
def run_game():
    ...
    max_camera_speed_world_units = PLAYER_CAR_MAX_SPEED_WORLD * CAMERA_SPEED_ASYMPTOTE_FACTOR
    ...
    title_screen_handler.set_high_score(
        high_score)  # Set initial high score for title

    game_over_screen_handler = GameOverScreen(screen_width,
                                              screen_height)
    stationary_obstacle_manager = ObstacleManager(screen_width,
                                                  screen_height,
                                                  world_unit_scale)
    bot_car_manager = BotManager(screen_width, screen_height,
                                 world_unit_scale,
    ...
    while is_running:
        ...
        if delta_time > (1.0 / 20.0): delta_time = (1.0 / 20.0)
        ...
        if current_game_state == STATE_TITLE:
            ...
            if player_car is None: current_game_state = STATE_TITLE; continue
            ...
            stationary_obstacle_manager.update_spawning_and_culling(
                game_camera_world_y)
            score_from_bots = bot_car_manager.update_bots(delta_time,
                                                          game_camera_world_y,
                                                          player_car.world_y)
            ...
            if game_over_reason:
                ...
                if current_score > high_score:
                    ...
                    is_new_high = True
                game_over_screen_handler.set_scores(current_score,
                                                    high_score,
                                                    is_new_high)
            ...
            game_background.draw(screen, game_camera_world_y,
                                 world_unit_scale)
            stationary_obstacle_manager.draw_all(screen,
                                                 game_camera_world_y,
                                                 world_unit_scale)
            bot_car_manager.draw_all(screen,
                                     game_camera_world_y)  # Draw bot cars
            ...
            if game_over_screen_handler.should_restart_game():
                current_game_state = STATE_TITLE
                title_screen_handler.set_high_score(
                    high_score)  # Update title screen with latest high score
                title_screen_handler.reset()
