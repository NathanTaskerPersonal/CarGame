...
COLOR_YELLOW = pygame.Color("yellow")  # For new high score
...
class GameOverScreen:
    def __init__(self, screen_width, screen_height):
        ...
        self.score_font = pygame.font.Font(None, 42)  # For scores
    ...
    def set_messages(self, main_msg=None, reason_msg=None,
                     prompt_msg=None):
        if main_msg is not None: self.main_message_text = main_msg
        if reason_msg is not None: self.reason_message_text = reason_msg
        # Prompt is handled separately or with default
        self.prompt_message_text = prompt_msg if prompt_msg is not None else \
            f"Hold SPACE for {RESTART_HOLD_TARGET_TIME:.0f}s to play again."

    def reset(self):
        self.space_held_duration = 0.0
        self.restart_game_triggered = False
        self.is_new_high_score = False  # Reset this flag

    def update(self, delta_time, keys_pressed):
        if self.restart_game_triggered: return
    ...
    def draw(self, surface):
        ...
        self._draw_text_line(surface,
                             ...
                             self.score_font, COLOR_WHITE, center_x,
                             current_y)
        current_y += self.score_font.get_height() + 50

        # Progress bar for restart
        progress_bar_width = 300
        progress_bar_height = 25
        progress_bar_x = (self.screen_width - progress_bar_width) // 2
        prompt_text_y = current_y  # Y for prompt text
        progress_bar_y = prompt_text_y + self.small_font.get_height() + 10

        pygame.draw.rect(surface, COLOR_PROGRESS_BAR_BG, (
        progress_bar_x, progress_bar_y, progress_bar_width,
        progress_bar_height))
        ...
        progress_bar_x, progress_bar_y, current_progress_fill,
        progress_bar_height))
