...
class Background:
    ...
    def draw(self, surface, camera_world_y, world_unit_scale):
        ...
        end_world_row_exclusive = math.ceil(
            (camera_pixel_y_offset + self.screen_height) /
            self.pixel_size
        )
        ...
        # It's good practice to fill the buffer once if there's
        # transparency or gaps or if not all pixels are redrawn
        # each frame (though here they are).
        ...
        for world_grid_y in range(start_world_row,
            ...
            for world_grid_x in range(num_world_cols_exclusive):
                ...
                screen_px_y_float = (
                    (world_grid_y * self.pixel_size) -
                    camera_pixel_y_offset
                )

                # Explicitly round to nearest int for drawing rects
                # world_grid_x * pixel_size is likely already int
                draw_x = round(screen_px_x_float)
