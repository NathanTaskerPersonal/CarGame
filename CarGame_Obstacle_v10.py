...
class Obstacle:
    ...
    def update_screen_rect(self, camera_world_y, world_unit_scale):
        """
        Updates self.rect to reflect the obstacle's current
        screen position.
        """
    ...
    def draw(self, surface, camera_world_y, world_unit_scale):
        ...
        # Only draw if the obstacle is potentially visible
        # (simple check)
        # A more robust check would involve checking intersection
        # with screen bounds
        if (self.rect.bottom > 0 and
                self.rect.top < surface.get_height()):
